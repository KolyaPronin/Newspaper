...
const IllustratorWorkspace: React.FC = () => {
  ...
  const [activeView, setActiveView] = useState<'illustrations' | 'tasks'>('illustrations');
  ...
  return (
    <div className="illustrator-workspace" style={{ maxWidth: 1400, margin: '0 auto', padding: '0 16px' }}>
      <div className="workspace-header">
        <div>
          <h1>{activeView === 'illustrations' ? 'Иллюстратор' : 'Мои задачи'}</h1>
          <p>{activeView === 'illustrations'
            ? 'Добавляйте изображения к одобренным статьям и загружайте рекламу'
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          <button
            type="button"
            className={`btn btn-auto ${activeView === 'illustrations' ? 'active' : ''}`}
