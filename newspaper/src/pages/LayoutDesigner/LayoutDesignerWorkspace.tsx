...
const LayoutDesignerWorkspace: React.FC = () => {
  ...
  const handleSubmitForReview = useCallback(async () => {
    // Collect all saved layout IDs across all pages
    const savedPages = Object.entries(pagesData)
      .filter(([, data]) => data.layoutId && data.layoutStatus === 'draft')
    ...
    try {
      ...
      setAutoSaveMessage('Весь выпуск отправлен на проверку');
    } catch (err) {
      setSaveError(err instanceof Error ? err.message : 'Ошибка отправки на проверку');
    } finally {
      setSubmitForReviewLoading(false);
    }
  }, [pagesData, setPagesData, setAutoSaveMessage, setSaveError]);
  ...
  return (
    <div className="layout-designer-workspace">
      ...
      <LayoutHeader
        ...
        layoutId={Object.values(pagesData).some(p => p.layoutId && p.layoutStatus === 'draft') ? 'has-draft' : null}
