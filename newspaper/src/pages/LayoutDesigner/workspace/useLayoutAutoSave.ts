...
export function useLayoutAutoSave({
  ...
  useEffect(() => {
    ...
    autoSaveTimeoutRef.current = setTimeout(async () => {
      try {
        const templateIdForSave = selectedTemplate.id;

        // Skip save if templateId is a local fallback (not a MongoDB ObjectId)
        if (!/^[a-f\d]{24}$/i.test(templateIdForSave)) {
          setAutoSaveMessage('Шаблон не загружен с сервера — сохранение недоступно');
          return;
        }
        ...
        newPagesData[currentPage] = {
          ...
          layoutStatus: saved.status,
          reviewComment: saved.reviewComment ?? null,
        };
