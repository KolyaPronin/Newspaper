...
const createIllustration = async (req, res) => {
  try {
    ...
    if (resolvedArticleId) {
      const articleForTask = await Article.findById(resolvedArticleId);
      if (articleForTask && articleForTask.issueId) {
        await taskService.checkAndCreateLayoutTask(articleForTask.issueId.toString(), req.user.id);
      }
    }
